...
分支约定：正式网页只从 `main` 开发和发布；`mobile-web` 仅保留手机版/PWA 实验，禁止整分支合并到 `main`。
...
白底抠图测试已经结束。remove.bg 和 PhotoRoom 兼容接口固定返回 410，不再读取密钥或调用第三方服务。
...
当前没有用户使用需求，功能和调查入口已停止。只有用户重新明确批准后才重新评估。
