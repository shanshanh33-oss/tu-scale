...
- 当前公开图片处理功能均在浏览器或用户本机完成，不上传图片内容。
- 白底 AI 抠图测试已经停止，线上代理接口不会再调用 remove.bg 或 PhotoRoom。
- 当前支付、积分和账号功能尚未实现。

## 分支策略

- `main` 是桌面和在线网页的唯一正式发布分支，Cloudflare Pages 只从 `main` 发布。
- `mobile-web` 只保留手机版/PWA 相关实验，不再作为网页正式功能的开发基线。
- `mobile-web` 中适合网页端的改动必须逐文件审查后选择性合入 `main`，禁止直接整分支合并。
...
- MediaPipe Tasks Vision 和浏览器端 Face Landmarker 模型
...
- Cloudflare KV
- 已停用的 remove.bg、PhotoRoom 兼容路由（固定返回功能已关闭）
...
当前已经包含 Node 自动化回归测试；尚未配置 CI、Docker 或 Wrangler。
...
- 智能锐化、自动色阶、自然饱和度和抗锯齿
- 放大前减少色块/伪影和抗摩尔纹
- 可独立开关的人脸关键点检测，以及可调皮肤净化程度的去伪影、抗摩尔纹和五官/毛孔纹理保护
- 浏览器端 AI 放大
- 原图和结果对比
- 单图下载和 ZIP 批量下载
- 设置保存和键盘快捷键
...
- remove.bg 和 PhotoRoom 兼容路由已经停用，不会调用第三方付费服务。
...
Vite 开发服务器保留以下旧接口代理到 `127.0.0.1:5180`，但当前公开前端不会调用抠图和问卷接口：
...
这些旧数据文件可能包含用户反馈、联系方式、IP 或文件名，不得提交到 Git。
